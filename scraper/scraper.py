...
# ── Здравеопазване: двуслойна класификация ────────────────────────────────
# STRONG: недвусмислено медицински предмет — печели винаги.
# WEAK: здравен контекст (болница като възложител и т.н.) — брои се само
#       ако няма EXCLUDE дума (немедицински предмет или ЗБУТ услуга).
_HEALTH_STRONG = ["лекарств", "фармацевт", "медицински изделия", "медицински консуматив",
                  "медицинска апаратура", "медицински апарати", "медицинска техника",
                  "реактив", "ваксин", "зъботехн", "зъболек", "превързочн",
                  "остеосинтеза", "кардиостимулац", "анестезия", "дезинфектант"]
_HEALTH_WEAK = ["здрав", "болниц", "мбал", "умбал", "дкц", "медицин", "клинич",
                "клиника", "диагностик", "дезинфек", "лаборатор"]
_HEALTH_EXCLUDE = ["условия на труд", "збут", "електрическа енергия", "електроенергия",
                   "ел. енергия", "канцеларски", "тонер", "охрана", "автомобил",
                   "автопарк", "сувенир", "ремонт", "строител", "отоплител",
                   "канализац", "перилни", "почистващ", "асансьор", "храни", "горива",
                   "на работещите", "на работниц", "на служителите", "застрахов",
                   "профилактични медицински прегледи", "задължителни медицински прегледи",
                   "периодични медицински прегледи", "трудова медицина"]

def _is_health(text):
    t = text.lower()
    if any(w in t for w in _HEALTH_STRONG):
        return True
    if any(w in t for w in _HEALTH_WEAK) and not any(x in t for x in _HEALTH_EXCLUDE):
        return True
    return False

def _eop_category(title, cpv=""):
    t = (title + " " + cpv).lower()
    # Силните здравни думи печелят преди всичко (напр. "апаратура за диагностика"
    # не бива да отива в "бизнес" заради "технолог")
    if any(w in t for w in _HEALTH_STRONG):
        return "здравеопазване"
    if any(w in t for w in ["дигитал", "цифров", "софтуер", "информационна система", "информационни технологии",
                             "информационно-технологичн", "киберсигурност", "ит систем"]):
        return "ит"
    if any(w in t for w in ["иновац", "технолог"]):
        return "бизнес"
    if any(w in t for w in ["образован", "обучен", "училищ", "детск"]):
        return "образование"
    if _is_health(t):
        return "здравеопазване"
    if any(w in t for w in ["социал"]):
        return "социални"
...
def parse_isun(soup, source):
    ...
    def get_category(text):
        t = text.lower()
        if any(w in t for w in ['дигитал', 'цифров', 'софтуер', 'киберсигурност', 'информационни технологии']):
            return 'ит'
        if any(w in t for w in ['иновац', 'конкурентоспособ', 'предприят']):
            return 'бизнес'
        if any(w in t for w in ['образован', 'обучен', 'училищ', 'висше', 'професионал', 'еразъм']):
            return 'образование'
        if _is_health(t):
            return 'здравеопазване'
        if any(w in t for w in ['социал', 'заетост', 'труд', 'деца', 'семейст', 'човешки ресурс']):
            return 'социални'
...
def parse_hrdc(soup, source):
    ...
    skip = ['отбелязв', 'напредък', 'шатра', 'отчита', 'резултат',
            ...
            'съобщен', 'новина', 'събитие', 'среща', 'разширява',
            'меморандум', 'координатор', 'партньорств']
    for a in soup.select('a'):
        text = a.get_text(strip=True)
        href = a.get('href', '')
        if not href or href.startswith('#') or href.startswith('mailto'):
...
def parse_mon(soup, source):
    ...
    skip = ['архив', 'изпълнителна агенция', 'програми и проекти',
            'регионални управления',
            '2019', '2020', '2021 –', '2021-', '2022 –', '2022-',
            '2023 –', '2023-', '2014-2020', 'приключил',
            'за 2024 г', 'за 2025 г', 'национални програми 2025',
            '2024 –', '2024-2027', '2025 –', '2021-2027', '2024 г',
            'национални програми 2026']
...
def parse_moew(soup, source):
    ...
    skip = ['приключил', 'архив', '2010', 'нормативни документи', 'обща информация',
            'програми и проекти', 'испа', 'швейцарска', 'статус на проект',
            'европейски и международни', '2014-2021', '2021-2027', '2024-2027',
            'проект по пос', 'в процес на изпълнение', 'норвежка програма']
...
def _normalize_title(title):
    """Нормализира заглавие за сравнение: малки букви, без кавички, без излишни интервали."""
    import re as _re
    title = title.lower()
...
def _title_similarity(a, b):
    """Дял на споделените думи (без стоп-думи) спрямо по-краткото заглавие."""
    stop = {'на', 'за', 'в', 'и', 'с', 'по', 'от', 'до', 'при', 'към', 'не', 'се', 'е',
            'или', 'а', 'но', 'да', 'са', 'им', 'го', 'я', 'му', 'ги', 'та'}
    wa = set(a.split()) - stop
    wb = set(b.split()) - stop
    if not wa or not wb:
        return 0.0
...
def enrich_eop_deadlines(programs):
    ...
    print("  [enrich] Зареждаме крайни срокове от openprocurements.com...")
